...
IFOE skills: `/validate-ifoe-exporter <user>@<host>` is the authoritative IFOE metric-correctness gate — cross-checks the `amdgpuifoe-exporter` `/metrics` endpoint against live `gpuctl` UAL state. Canonical form for agentq dispatch is [`.claude/skills/validate-ifoe-exporter/SKILL.md`](.claude/skills/validate-ifoe-exporter/SKILL.md).
