...
- `IFOEConfig`:
  ...
  - `Labels`: An array of label keys to include in exported metrics. The following labels are always exported regardless of this array: `hostname`, `gpu_uuid`, `device_uuid`, `station_uuid`, `port_name`, `port_index`, `station_index`, `accelerator_id`, `vpod_id`, and `physical_pod_id`. See the [IFOE Metrics List](ifoe-metricslist.md#ifoe-metric-labels) for a description of each. Optional labels can be added by including their config key names (uppercase) in this array, for example: `HOSTNAME`, `GPU_UUID`, `CARD_SERIES`, `CARD_MODEL`, `CARD_VENDOR`, `DRIVER_VERSION`, `VBIOS_VERSION`, `SERIAL_NUMBER`, `POD`, `NAMESPACE`, `CONTAINER`, `JOB_ID`, `JOB_USER`, `JOB_PARTITION`, `CLUSTER_NAME`, `POD_UUID`. Supported labels are listed in the provided example `configmap.yml`.
